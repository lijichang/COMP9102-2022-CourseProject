...
def centroid_triplet_eval(X, X_new, y):
    ...
    for i in range(num_cat):
        label = categories[i]
        X_clus_ori = X[y.reshape(-1) == label.reshape(-1)]
        X_clus_new = X_new[y.reshape(-1) == label.reshape(-1)]
...
def evaluate_output(X, X_new, y, name, baseline=True, labelled=True):
    results = {}
    results['name'] = name
    if labelled:
        if baseline:
...
def generate_combined_figure(embeddings, labels, titles, theme_title):
    ...
    with PdfPages("{}.pdf".format(theme_title)) as pdf:
